...
const server = http.createServer(async (req, res) => {
  if (req.method === 'POST' && req.url === '/post-with-image') { let b2=''; req.on('data',c=>b2+=c); req.on('end',async()=>{ try{ const {message,imagePrompt}=JSON.parse(b2); const {OpenAI}=require('openai'); const oa=new OpenAI({apiKey:process.env.OPENAI_API_KEY}); const img=await oa.images.generate({model:'dall-e-3',prompt:imagePrompt,n:1,size:'1024x1024'}); const pr=await axios.post('https://graph.facebook.com/v25.0/'+PAGE_ID+'/photos',{url:img.data[0].url,caption:message,access_token:PAGE_ACCESS_TOKEN,published:true}); res.writeHead(200,{'Content-Type':'application/json'}); res.end(JSON.stringify({success:true,post_id:pr.data.id})); }catch(e){ res.writeHead(500,{'Content-Type':'application/json'}); res.end(JSON.stringify({success:false,error:e.message})); } }); } else if (req.method==='POST' && req.url==='/post') {
    let body = '';
